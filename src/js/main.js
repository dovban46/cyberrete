...
document.addEventListener('DOMContentLoaded', function() {
  ...
  sections.forEach((section) => {
    const tabs = section.querySelectorAll('[data-faq-tab-index]');
    const panels = section.querySelectorAll('[data-faq-panel-index]');
    ...
    const closeAllItems = (panel, exceptItem = null) => {
    ...
    const switchToPanel = (index) => {
      const activePanel = section.querySelector('.faq__panel.is-active');
      const nextPanel = section.querySelector(`.faq__panel[data-faq-panel-index="${index}"]`);
      if (!nextPanel || activePanel === nextPanel) return;

      tabs.forEach((tab) => {
        const isActive = String(index) === tab.getAttribute('data-faq-tab-index');
      ...
      setTimeout(() => {
        if (activePanel) {
          activePanel.classList.remove('is-active', 'is-transitioning-out');
    ...
    const openPanel = (index) => {
      ...
      panels.forEach((panel) => {
        const isActive = String(index) === panel.getAttribute('data-faq-panel-index');
        panel.classList.toggle('is-active', isActive);
        panel.hidden = !isActive;
        if (!isActive) closeAllItems(panel);
      });
    };

    tabs.forEach((tab) => {
      tab.addEventListener('click', () => {
        const index = tab.getAttribute('data-faq-tab-index');
        switchToPanel(index);
      });
    });

    panels.forEach((panel) => {
      panel.querySelectorAll('.faq__question').forEach((questionBtn) => {
    ...
    section.querySelectorAll('.faq__answer-wrap').forEach((answer) => {
      answer.hidden = false;
    });
    ...
    window.addEventListener('resize', () => {
      section.querySelectorAll('.faq__item.is-open .faq__answer-wrap').forEach((answer) => {
        answer.style.setProperty('--faq-answer-height', `${answer.scrollHeight}px`);
      });
    });
  });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.querySelectorAll('.js-about-team-slider').forEach((sliderEl) => {
    const root = sliderEl.closest('.about-page-team__container');
    if (!root) return;

    const prevBtns = root.querySelectorAll('.js-about-team-prev');
    const nextBtns = root.querySelectorAll('.js-about-team-next');
    const paginationEl = sliderEl.querySelector('.about-page-team__pagination');
    if (!prevBtns.length || !nextBtns.length || !paginationEl) return;
    ...
    const swiper = new Swiper(sliderEl, {
      ...
      navigation: {
        prevEl: prevBtn,
        nextEl: nextBtn,
        disabledClass: 'about-page-team__nav-btn--disabled'
      },
      pagination: {
        el: paginationEl,
        clickable: true,
    ...
    const syncNav = () => {
      ...
      prevBtns.forEach((btn) => {
        btn.classList.toggle('about-page-team__nav-btn--disabled', dPrev);
        btn.setAttribute('aria-disabled', dPrev ? 'true' : 'false');
      });
      nextBtns.forEach((btn) => {
        btn.classList.toggle('about-page-team__nav-btn--disabled', dNext);
        btn.setAttribute('aria-disabled', dNext ? 'true' : 'false');
      });
    };
    ...
    swiper.on('reachBeginning', syncNav);
    swiper.on('reachEnd', syncNav);
    swiper.on('fromEdge', syncNav);
    swiper.on('resize', syncNav);
    swiper.on('afterInit', syncNav);
  });
